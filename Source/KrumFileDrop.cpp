...
void KrumFileDrop::filesDropped(const juce::StringArray& files, int x, int y)
{
    ...
    for (auto file : files)
    {
        ...
        {
            createNewModule(audioFile, audioFile.getFileName());
...
void KrumFileDrop::createNewModule(juce::File audioFile, juce::String name)
{
    //int index = container.findFreeModuleIndex();
    int index = editor.getSampler().getNumModules();
    editor.createModule(name, index, audioFile);
}
